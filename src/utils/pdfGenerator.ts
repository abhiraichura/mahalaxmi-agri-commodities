...
export const generateContractPDF = (
  ...
  // First party (left column) - ONLY show fields that are safe for contracts
  let py = y + 5;
  doc.setFont('helvetica', 'bold');
  doc.setFontSize(8);
  doc.setTextColor(...RED);
  ...
  py += 3.5;
  // STRICT: Only legal business fields - NO contactPerson, alternateNumbers, alternateEmails, contacts
  doc.text(`GSTIN: ${firstParty.gstin} | Phone: ${firstParty.phone || 'N/A'}`, M + 3, py);

  // Second party (right column) - ONLY show fields that are safe for contracts
  const rx = M + colW + 4;
  py = y + 5;
  doc.setFont('helvetica', 'bold');
  doc.setFontSize(8);
...
export const generateBrokerageBillPDF = (
  ...
  // Handle both monthly and date range bills
  if (bill.isRangeBill) {
    doc.text(`Period: ${bill.fromDate} to ${bill.toDate}`, M, y);
  } else {
  ...
  doc.setFontSize(8);
  doc.setTextColor(...GRAY);
  // STRICT: Only GSTIN shown, NO contactPerson, alternateNumbers, alternateEmails, contacts
  ...
  if (bill.status === 'paid') {
    ...
    doc.text('PAID', M, finalY);
    if (bill.paymentDate) {
      doc.setFontSize(8);
      doc.setTextColor(...GRAY);
      doc.text(`Payment Date: ${bill.paymentDate}`, M, finalY + 5);
    }
    if (bill.paymentMode) {
      doc.text(`Mode: ${bill.paymentMode}`, M, finalY + 9);
    }
    if (bill.paymentReference) {
      doc.text(`Ref: ${bill.paymentReference}`, M, finalY + 13);
    }
  }
